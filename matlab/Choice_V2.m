function [Peo_pos0_n, Pos_save0, V0_n, Stop_tn, Peo_M0n, Pos_start0n]=Choice_V2(M_0,Pos_start0,Peo_pos0,Pos_save0,V0,Stop_t,N_in0)
global Num_per2 N_in_ant2 N_num_ant2 Num_stair Num_sq2 Pos_stair M_stair
...
if p<50 && Num_sq2(q)<25
    Peo_M0n(Peo_pos0_n(i,1),Peo_pos0_n(i,2))=0;
    N_num_ant2(q)=N_num_ant2(q)+1;
    A=Pos_save0(i).pos;
    N_in_ant2(q,N_num_ant2(q)).roadmat=A;
    M_ant_upd2(q,A)
    
    Num_sq2(q)=Num_sq2(q)+1;
    ...
    for x=1:1:5
        for y=1:1:5
            ...
            if M(x,y)==0 && lambda==0
                M_stair(q).M(x,y)=1;
                Pos_stair(q).pos(Num_stair(q),:)=[x y];
                lambda=1;
            end
        end
    end
